...
public enum ErrorCode {
    ...
    ADMIN_ACCOUNT_PENDING_APPROVAL(HttpStatus.FORBIDDEN,
            "권한 승인을 기다리는 계정입니다. 전체 관리자의 승인 후 로그인할 수 있습니다."),
    ADMIN_ACCOUNT_ALREADY_HAS_PASSWORD(HttpStatus.CONFLICT,
            "이미 비밀번호를 정한 계정입니다. 초대 재전송으로는 비밀번호를 바꿀 수 없습니다."),
    ADMIN_ACCOUNT_NOT_AWAITING_APPROVAL(HttpStatus.CONFLICT,
            "승인을 기다리는 계정이 아닙니다."),
    SURVEY_PHOTO_LIMIT_EXCEEDED(HttpStatus.BAD_REQUEST, "사진을 더 올릴 수 없습니다."),
    SURVEY_CHANNEL_CLOSED(HttpStatus.NOT_FOUND, "지금은 열려 있지 않은 판매 경로입니다."),
    SURVEY_PICKUP_NOT_AVAILABLE(HttpStatus.BAD_REQUEST, "현장 수령을 고를 수 없는 주문입니다."),
    SURVEY_PHOTO_COUNT_INVALID(HttpStatus.BAD_REQUEST, "사진은 아이마다 1장부터 5장까지 올릴 수 있습니다."),
    SURVEY_PHOTO_WARNING_REQUIRED(HttpStatus.BAD_REQUEST, "사진이 부족할 때 보여 드린 안내를 확인해 주세요."),
    SURVEY_PHOTO_SHARED_BETWEEN_PETS(HttpStatus.BAD_REQUEST, "같은 사진을 여러 아이에게 나누어 쓸 수 없습니다."),
    SURVEY_PET_COUNT_INVALID(HttpStatus.BAD_REQUEST, "아이는 한 번에 1마리부터 5마리까지 신청할 수 있습니다."),
