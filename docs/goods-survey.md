...
4. 예약된 응답만 사연, 비공개 사진, 제작·배송 정보를 제출할 수 있습니다. 사진별 외부 공개 동의는 `publicPhotoIds`로 별도 저장하며, 최종 제출은 `Idempotency-Key`로 중복 처리되지 않습니다.

설문 답변, 사연, 제작·배송 개인정보, 사진 메타데이터는 서로 다른 테이블에 저장합니다. 연락처와 주소는 AES-GCM으로 암호화하고 연락처 중복 확인에는 캠페인별 HMAC blind index만 사용합니다.

현재 질문지 버전은 `2026-07-25-v2`입니다. 무중단 전환을 위해 직전 버전 `2026-07-23-v1` 요청도 서버에서 임시 허용하며, 새 프런트 배포가 안정화된 뒤 레거시 허용 목록에서 제거합니다.
