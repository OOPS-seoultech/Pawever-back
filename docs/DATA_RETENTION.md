...
| 대상                           | 기간         | 세는 기준                 | 설정                           |
| ------------------------------ | ------------ | ------------------------- | ------------------------------ |
| 제작용 사진, 상세주소          | 한국 시간 달력상 3개월 | **배송 완료를 표시한 날** | `personal-data-retention-months` |
...
- 공개에 동의한 사진은 배송 완료 후 3개월에 지우지 않는다. 제작이 아니라 공개가 목적이라
  설문과 같은 2년을 따르고, 그때 함께 지운다.
- **3개월에 신청 정보를 통째로 지우지 않는다.** 유료 판매는 전자상거래법이
  대금 결제와 재화 공급 기록을 5년 보존하도록 한다. 3개월에는 사진과 상세주소만
...
## 배송 완료를 표시해야 3개월이 시작된다

**표시하지 않으면 배송 정보가 계속 남는다.** 기간을 셀 기준일이 없기 때문이다.

관리자 화면에서 끝낸 주문은 표시가 저절로 된다.

- 택배 주문: **송장 등록**(`POST /api/admin/orders/{orderNumber}/tracking`)이
  발송 완료로 옮기면서 기준일을 잡는다. 송장을 고쳐 다시 넣어도 날짜는
  밀리지 않는다.
- 현장 수령 주문: **수령 완료**(`POST /api/admin/orders/{orderNumber}/pickup-complete`)가
  `PICKED_UP` 으로 옮기면서 같은 기준일을 잡는다.

관리자 화면을 거치지 않고 보낸 건(1차 체험단을 화면 밖에서 처리한 경우 등)만
아래를 건마다 호출한다.
...
## 계약이 없어진 주문은 그 자리에서 지운다

결제 대기가 시간을 넘겨 저절로 만료되면 스케줄러가 사진과 상세주소를 바로
지운다. 관리자가 손으로 **결제 만료·결제 실패**로 옮기거나 **주문을 취소**한
경우도 같다 — 같은 상태면 같은 일이다. 행과 이력은 남기고 사진과 상세주소만
지우므로, 무엇이 왜 만료·취소됐는지는 관리자 화면에서 그대로 볼 수 있다.

**취소 처리 실패**는 지우지 않는다. 돈은 받아 둔 채 환불에 실패한 상태라
사람이 정리하기 전까지 그 물건은 그 사람 몫이다.
